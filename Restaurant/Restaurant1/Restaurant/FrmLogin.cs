...
namespace Restaurant
{
    public partial class FrmLogin : Form
    {
        ...
        /// <param name="sender"></param>
        #region 登录方法
        private void Login(Button sender)
        {
            if (CheckNull())
            {
                bool flag=false;//是否为店长登录
                Admin ad = new Admin();
                ad.LoginId = cboName.Text;
                ad.LoginPwd = txtPwd.Text;
                if (sender.Name.Equals("btnBoss"))
                {
                    ad.PersonnelLevel = 1;
                    flag = true;
                }
                else if(sender.Name.Equals("btnLogin"))
                {
                    ad.PersonnelLevel = 2;
                }
        ...
        /// </summary>
        #region 记录账号密码到本地
        private void JiLu(Button sender)
        {
            Admin adm = new Admin();
            ...
            if (sender.Name.Equals("btnBoss"))
            {
                adm.PersonnelLevel = 1;
            }
            else if (sender.Name.Equals("btnLogin"))
            {
                adm.PersonnelLevel = 2;
            }
